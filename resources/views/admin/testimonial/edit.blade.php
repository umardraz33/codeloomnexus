    ...
    <div class="content sm-gutter">
      ...
      <div class="page-title">
        ...
        <div class="pull-right">
          <a href="{{ route('testimonial.index') }}" class="btn btn-success" type="submit">Back</a>
        </div>
      </div>
        ...
        <div class="col-md-8">
          <div class="grid simple form-grid">
            <div class="grid-title">
                <h4>Edit <span class="semi-bold"> testimonial</span></h4>

            </div>
            <div class="grid-body no-border">
              <form action="{{ route('testimonial.update' , $testimonial->id) }}" method="POST" enctype="multipart/form-data"  autocomplete="off">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label class="form-label">Name</label>
                    <input class="form-control" id="form1Amount" name="name" type="text" value="{{ $testimonial->name }}">
                  </div>
                <div class="form-group">
                  <label class="form-label">Title</label>
                  <input class="form-control" id="form1Amount" name="title" type="text" value="{{ $testimonial->title }}">
                </div>
                <div class="form-group">
                    <label class="form-label">Image</label>
                    <input class="form-control" id="form1Amount" name="image" type="file" value="{{ $testimonial->image }}"><span>{{ $testimonial->image }}</span>
                </div>
                <div class="form-group">
                  <label class="form-label">Comment</label>
                  <textarea class="form-control" name="comment" id="comment" cols="30" rows="4">{{ $testimonial->comment }}</textarea>
                </div>
                <div class="form-group">
                    <div class="checkbox checkbox check-success 	">
                        <input type="checkbox" name="is_active" value="1" {{ $testimonial->is_active == 1 ? 'checked' : '' }} id="chkTerms">
